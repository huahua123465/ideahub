...
2. 运行 `git fetch origin --prune`，识别当前分支名，并把它应有的远端固定为 `origin/<当前分支>`。
3. 只有同名远端分支存在时，才确认 upstream 与 `origin/<当前分支>` 完全一致并执行无参数 `git pull --ff-only`。同名远端不存在说明这是尚未发布的新分支；不得把创建分支时继承的 `origin/main` 当作当前分支 upstream，也不得靠拉 main 冒充同步当前分支。
4. 单独检查 `origin/main` 是否为当前 `HEAD` 的祖先。不是祖先时停止修改，先决定如何安全接入 main；不要让 `git pull` 隐式生成合并提交。
5. 已有的用户改动不覆盖、不丢弃、不替用户提交；需要绕开时明确说明。
6. 先判断任务是否涉及数据库、`.env`、依赖、Docker/Caddy、端口/域名、Collector、生产数据或线上操作，再加载对应 skill。
7. 默认只在本地工作。没有用户明确要求和相应权限，不连接、不修改生产环境。
