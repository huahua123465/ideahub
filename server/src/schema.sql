...
CREATE TABLE IF NOT EXISTS account_research_evidence_locations (
  ...
  CONSTRAINT account_research_evidence_locations_asset_fk FOREIGN KEY(sample_id,asset_id)
    REFERENCES sample_assets(sample_id,id) ON DELETE RESTRICT,
  CONSTRAINT account_research_evidence_locations_profile_fk FOREIGN KEY(profile_snapshot_id)
    REFERENCES research_account_profile_snapshots(id) ON DELETE RESTRICT,
...
SET status='failed',warnings_json=CASE WHEN warnings_json ? 'legacy_v1_build_aborted' THEN warnings_json ELSE warnings_json||'["legacy_v1_build_aborted"]'::jsonb END,
...
ALTER TABLE account_research_claim_samples ADD CONSTRAINT account_research_claim_samples_role_chk CHECK(role IN ('eligible','present','representative','counterexample'));
ALTER TABLE account_research_claims DROP CONSTRAINT IF EXISTS account_research_claims_pattern_chk;
ALTER TABLE account_research_claims ADD CONSTRAINT account_research_claims_pattern_chk CHECK(pattern_code IS NULL OR pattern_code ~ '^[a-z0-9_]{3,64}$');
ALTER TABLE account_research_claims DROP CONSTRAINT IF EXISTS account_research_claims_content_goal_chk;
ALTER TABLE account_research_claims ADD CONSTRAINT account_research_claims_content_goal_chk CHECK((dimension_key='content_supply' AND claim_type<>'insufficient' AND content_goal IN ('traffic','persona','expertise','relationship','conversion','mixed')) OR ((dimension_key<>'content_supply' OR claim_type='insufficient') AND content_goal IS NULL));
CREATE UNIQUE INDEX IF NOT EXISTS account_research_claims_run_dimension_pattern_uk ON account_research_claims(run_id,dimension_key,pattern_code) WHERE pattern_code IS NOT NULL;
...
CREATE OR REPLACE FUNCTION account_research_run_guard() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE sample_count INT;sample_distinct_ordinals INT;sample_min_ordinal INT;sample_max_ordinal INT;claim_count INT;dimension_count INT;invalid_dimensions INT;invalid_memberships INT;outside_count INT;null_patterns INT;BEGIN
 IF TG_OP='DELETE' OR OLD.status IN('complete','failed')THEN RAISE EXCEPTION 'terminal account research runs are immutable' USING ERRCODE='55000';END IF;
 IF(to_jsonb(NEW)-ARRAY['status','completed_at','content_matrix_json','saturation_json'])IS DISTINCT FROM(to_jsonb(OLD)-ARRAY['status','completed_at','content_matrix_json','saturation_json'])THEN RAISE EXCEPTION 'account research run inputs are immutable' USING ERRCODE='55000';END IF;
 IF NEW.status='complete'THEN SELECT count(*),count(DISTINCT ordinal),min(ordinal),max(ordinal)INTO sample_count,sample_distinct_ordinals,sample_min_ordinal,sample_max_ordinal FROM account_research_run_samples WHERE run_id=NEW.id;SELECT count(*),count(DISTINCT dimension_key)INTO claim_count,dimension_count FROM account_research_claims WHERE run_id=NEW.id;
  ...
  IF NEW.schema_version='account-research/1.1'THEN SELECT count(*)INTO invalid_dimensions FROM(SELECT dimension_key FROM account_research_claims WHERE run_id=NEW.id GROUP BY dimension_key HAVING count(*)NOT BETWEEN 1 AND 5)q;
   SELECT count(*)INTO null_patterns FROM account_research_claims WHERE run_id=NEW.id AND pattern_code IS NULL;IF claim_count NOT BETWEEN 8 AND 40 OR dimension_count<>8 OR invalid_dimensions<>0 OR null_patterns<>0 THEN RAISE EXCEPTION 'complete account research 1.1 run requires 1-5 claims in all eight dimensions (claims %, dimensions %, invalid %, null patterns %)',claim_count,dimension_count,invalid_dimensions,null_patterns USING ERRCODE='23514';END IF;
   SELECT count(*)INTO invalid_memberships FROM account_research_claims c WHERE c.run_id=NEW.id AND(c.eligible_count<>(SELECT count(*)FROM account_research_claim_samples cs WHERE cs.claim_id=c.id AND cs.role='eligible')OR c.present_count<>(SELECT count(*)FROM account_research_claim_samples cs WHERE cs.claim_id=c.id AND cs.role='present')OR EXISTS(SELECT 1 FROM account_research_claim_samples cs WHERE cs.claim_id=c.id AND cs.role IN('present','representative','counterexample')AND NOT EXISTS(SELECT 1 FROM account_research_claim_samples e WHERE e.claim_id=c.id AND e.sample_id=cs.sample_id AND e.role='eligible'))OR EXISTS(SELECT 1 FROM account_research_claim_samples cs WHERE cs.claim_id=c.id AND cs.role='representative'AND NOT EXISTS(SELECT 1 FROM account_research_claim_samples p WHERE p.claim_id=c.id AND p.sample_id=cs.sample_id AND p.role='present'))OR EXISTS(SELECT 1 FROM account_research_claim_samples cs WHERE cs.claim_id=c.id AND cs.role='counterexample'AND EXISTS(SELECT 1 FROM account_research_claim_samples p WHERE p.claim_id=c.id AND p.sample_id=cs.sample_id AND p.role='present')));
   IF invalid_memberships<>0 THEN RAISE EXCEPTION 'claim sample counts or subset memberships invalid' USING ERRCODE='23514';END IF;SELECT count(*)INTO outside_count FROM account_research_run_samples s WHERE s.run_id=NEW.id AND s.published_at IS NOT NULL AND(s.published_at<NEW.observation_start OR s.published_at>NEW.observation_end);IF outside_count<>0 THEN RAISE EXCEPTION 'run sample outside observation window' USING ERRCODE='23514';END IF;
   IF NEW.content_matrix_json IS NULL OR jsonb_typeof(NEW.content_matrix_json)<>'object' OR NEW.saturation_json IS NULL OR jsonb_typeof(NEW.saturation_json)<>'object' OR NEW.saturation_json->>'ruleVersion'<>'saturation/1.0'THEN RAISE EXCEPTION 'complete account research 1.1 run requires matrix and saturation' USING ERRCODE='23514';END IF;
   IF NOT account_research_validate_depth(NEW.id,NEW.content_matrix_json,NEW.saturation_json)THEN RAISE EXCEPTION 'persisted matrix or saturation does not match frozen research members' USING ERRCODE='23514';END IF;
...
-- ---------- 报销审批（2026-09-11） ----------
-- 流程：申请人提交 → 部门负责人 → 总经理 → 财务 → 出纳打款 → 申请人确认收款。
...
--   received_at：申请人确认收款的时间；reminded_at：最近一次提醒申请人确认收款的时间
--   （completed / received_at / reminded_at 2026-09-15 加，迁移见 scripts/migrations/20260915-expense-receipt-confirm.sql）
...
--   confirm 申请人确认收到钱 | dispute 申请人反馈没收到，单子回到出纳（2026-09-15 加）
-- revoke 之后，那一步及后面各步在它之前的记录不再参与流转计算，但保留在这张表里；
-- dispute 记在出纳这一步，按同样规则让之前那次打款不再算数。
...
-- ---------- 采购申请（2026-09-14） ----------
-- 迁移见 scripts/migrations/20260914-purchase-requests.sql，两边保持一致。
...
-- 附件复用 attachments（scope = 'purchase'，ref_id = 采购单 id）。
